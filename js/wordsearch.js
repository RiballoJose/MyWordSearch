H5P.WordSearch.CrossWord = function (parameters, id) {
	...
	this.getSquareGrid = function(max_tries, min_size){
		var best_grid = null;
		var best_ratio = 0;
		
		for (var i = 0; i < max_tries; i++){
			var a_grid = this.getGrid(1, min_size);
	...
	this.getGrid = function (max_tries, min_size){
		var groups = [];
		for(var tries = 0; tries < max_tries; tries++){
			...
			if(word_has_been_added_to_grid) return minimizeGrid(min_size);  
		}
		return null;
	}
			...
			for(var c = 0; c < grid[r].length; c++){
				...
				for(var k in groups){
					...
					if(cell && cell[k] && cell[k]['is_start_of_word']){
						groups[k].push({"position" : position, "index" : cell['word_index'], "word" : words_in[cell[k]['index']]});
	...
	var minimizeGrid = function(min_size){
		// find bounds
	...
	var addCellToGrid = function(word, index_of_word_in_input_list, index_of_char, r, c, direction){
		...
		if(grid[r][c] == null){
			grid[r][c] = new H5P.WordSearch.CrosswordCell(word, char, index_of_word_in_input_list);
		...
		grid[r][c][direction] = new H5P.WordSearch.CrosswordCellNode((index_of_char == 0),(index_of_char == word.length-1), index_of_word_in_input_list);
